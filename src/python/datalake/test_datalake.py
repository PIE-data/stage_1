...
from src.python.datalake.book_storage import BookBasedStorage
from src.python.datalake.batch_storage import BatchBasedStorage
"""
#### ATOMIC WRITE TESTS #####
"""
...
"""
##### Book Based Storage TEST #####
"""

def test_book_storage_roundtrip(tmp_path: Path):

    storage = BookBasedStorage(workspace=tmp_path)
    book_id = 1342
    header_content = "Title: Pride and Prejudice\nAuthor: Jane Austen\n"
    body_content = "It is a truth universally acknowledged...\n"
    ...
    assert rel_header == "datalake/books/1342/header.txt"
    assert rel_body == "datalake/books/1342/body.txt"
...
def test_book_storage_lookup_missing(tmp_path: Path):
    storage = BookBasedStorage(workspace=tmp_path)
    assert storage.lookup(99999) is None

def test_book_storage_list_new(tmp_path: Path):
    import time
    storage = BookBasedStorage(workspace=tmp_path)

    storage.write(1, "Header 1", "Body 1")

    time.sleep(0.05)
    t_mid = datetime.now(timezone.utc)
    time.sleep(0.05)

    storage.write(2, "Header 2", "Body 2")

    new_books = list(storage.list_new(t_mid))
    assert new_books == [2]

"""
##### Batch Based Storage TESTS
"""

def test_batch_storage_roundtrip(tmp_path: Path):
    storage = BatchBasedStorage(workspace=tmp_path)
    book_id = 1342
    header_content = "Title: Pride and Prejudice\nAuthor: Jane Austen\n"
    body_content = "It is a truth universally acknowledged...\n"
    ...
    assert rel_header == "datalake/00/13/1342.header.txt"
    assert rel_body == "datalake/00/13/1342.body.txt"
...
def test_batch_storage_lookup_missing(tmp_path: Path):
    storage = BatchBasedStorage(workspace=tmp_path)
    assert storage.lookup(99999) is None

def test_batch_storage_list_new(tmp_path: Path):
    """SPEC.md §4.1: list_new(since) filters by mtime."""
    import time
    storage = BatchBasedStorage(workspace=tmp_path)
    ...
    all_books = sorted(list(storage.list_new(t_early)))
    assert all_books == [1, 2]
