...
from src.python.datalake.downloader import HEADERS, SESSION, download
...
def test_download_success():
    ...
    with patch.object(SESSION, "get", return_value=mock_response) as mock_get:
        ...
        assert SESSION.headers["User-Agent"] == HEADERS["User-Agent"]
...
def test_download_retries_on_500_and_recovers():
    ...
    with patch.object(SESSION, "get", side_effect=[res_500, res_200]) as mock_get:
...
def test_download_exhausted_retries_raises_runtime_error():
    ...
    with patch.object(SESSION, "get", return_value=res_500) as mock_get:
        with patch("time.sleep") as mock_sleep:
            with pytest.raises(RuntimeError):
                download(123)
            # SPEC.md §2.1: first attempt + 3 retries, no wait after the last.
